...
/// @brief Make a context object based on a given parameter preset with
/// considering slot-wise ring packing into a larger ring degree in NTT state
HEAAN_API Context makeNttEmbeddingContext(
    const ParameterPreset &preset, const std::string &param_name,
    const u64 ntt_embedding_log_ratio, const u64 num_sk, const bool is_ci_param,
    const bool is_emb_param, const CudaDeviceIds &cuda_device_ids = {});
